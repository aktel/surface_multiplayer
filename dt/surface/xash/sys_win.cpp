...
std::vector<std::string> g_sLibsBowing;
 
qboolean extern "C" Sys_AllowLibararyBorrowing( const char *dll_name, qboolean remove )
{
	for ( std::vector<std::string>::iterator it = g_sLibsBowing.begin(); it <= g_sLibsBowing.end(); ++it )
	{
		if ( Q_stricmp( it.c_str(), dll_name ) )
		{
			if ( remove )
			{
				*it = *g_sLibsBowing.end();
				g_sLibsBowing.pop_back();
			}
...
void extern "C" Sys_LibararyBorrowing( const char *dll_name )
{
	if ( Sys_AllowLibararyBorrowing( dll_name, false ) )
		return;

	g_sLibsBowing.push_back( std::string(dll_name) );
}

BEGIN_C

void Sys_SetSafeLibrary( qboolean state )
{
	static syshook_t loadlibrary;
	static qboolean oldstate = false;
	...
	if ( state )
		Sys_Hook( "LoadLibrary", &Sys_LoadLibrary, HK_EXTRA, &loadlibrary );
	else Sys_Unhook( &loadlibrary );
}
...
//=======================================================================
qboolean Sys_LoadLibrary( dll_info_t *dll, qboolean subhack )
{
	...
	if( !dll->link ) 
	{
		if ( !subhack || !Sys_FindLibrary( dll ) )
		{
	...
	if ( subhack )
		Sys_LibararyBorrowing( dll->name );
...
qboolean Sys_FindLibrary( dll_info_t *dll )
{
	if ( !dll ) return false;

	dll->link = GetModuleHandle( dll->name );

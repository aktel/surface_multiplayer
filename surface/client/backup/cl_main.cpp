...
void CL_ComputePacketLoss( void )
{
	...
	if( host.realtime < cls.local.packet_loss_recalc_time )
		return;

	// recalc every second
	cls.local.packet_loss_recalc_time = host.realtime + 1.0;

	// compuate packet loss
	for( i = cls.local.netchan.incoming_sequence - CL_UPDATE_BACKUP+1; i <= cls.local.netchan.incoming_sequence; i++ )
	{
		frm = i;

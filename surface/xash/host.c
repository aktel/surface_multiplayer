...
=================
*/
void Host_Frame( float time, eHostProcess process )
{
	if( setjmp( host.abortframe ))
		return;

	if ( process & FRAME_SEGMENT_INPUT )
		Host_InputFrame ();	// input frame

	// decide the simulation time
	if ( !(process & FRAME_SEGMENT_GAMECALL) )
		if( !Host_FilterTime( time ))
			return;

	Host_GetConsoleCommands ();

	Host_ServerFrame ( process ); // server frame
	Host_ClientFrame ( process ); // client frame

	if ( process & FRAME_SEGMENT_INFO )
		host.framecount++;
}
...
=================
*/
int Host_Main( const char *progname )
{
...
void Host_Process( eHostProcess process )
{
	...
	while( !host.crashed )
	{
		...
		Host_Frame( newtime - oldtime, process );
		oldtime = newtime;
	}
}
